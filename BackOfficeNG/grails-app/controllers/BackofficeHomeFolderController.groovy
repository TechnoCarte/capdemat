...
class BackofficeHomeFolderController {
    ...
    def child = {
        ...
        if (request.post && !child.id) {
            mode = 'static'
            bind(child)
            child.homeFolder = responsible.homeFolder
            if (!params.roleType.isEmpty()) {
                homeFolderService.addChild(responsible.homeFolder, child)
                homeFolderService.link(responsible, child, [RoleType.forString(params.roleType)])
            }
            def invalidFields = individualService.validate(child)
            if (!invalidFields.isEmpty())  {
                session.doRollback = true
                def errors = []
                invalidFields.each{ errors.add(it = it.replace('legalResponsibles','roleType'))}
        ...
        if (!child.id) {
            models['responsible'] = responsible
        } else {
